                            ...
                            <div class="flex justify-between items-center border-t pt-4">
                                ...
                                <div class="flex space-x-3 items-center">
                                    ...
                                    @if(Auth::id() == $item->user_id || Auth::user()->role == 'admin')
                                        <form action="{{ route('lost-found.destroy', $item) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus laporan ini?');">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    @if($item->status == 'pending' && (Auth::user()->role == 'admin' || Auth::user()->role == 'satpam'))
                                        <form action="{{ route('lost-found.update-status', $item) }}" method="POST">
                                            ...
                                            <button type="submit" class="text-green-600 hover:text-green-900 text-xs font-semibold">Verifikasi</button>
                                        </form>
                                    @endif
